...
class MultiListener:
    ...
    def __init__(self, list_of_listeners, spotify_account):
        ...
        if self.main_user.spotipy_object_playlist_creation is None:
            warnings.warn(
                "User_id for in spotify_account is missing. playlist creation will not be possible."
            )
    ...
    def rank_artists(self):
        ...
        for listener in self.list_of_listeners:
            for item in listener.top_artists["items"]:
                ...
                for related_artist in self.main_user.spotipy_object.artist_related_artists(
                        artist_id)["artists"]:
    ...
    def create_genre_dict(self):
        ...
        # not all all genres
        all_genres = [
            "pop", "metal", "k-pop", "hip hop", "rock", "indie", "rap", "alt z",
            "reggae", "low-fi"
        ]
        for listener in self.list_of_listeners:
            for item in listener.top_artists["items"]:
    ...
    def save_playlist(self, tracks_list, name=None):
        ...
        playlist = self.main_user.spotipy_object_playlist_creation.user_playlist_create(
            user=self.main_user.user_id,
            name=name,
            public=True,
            collaborative=False,
            description='')
        self.main_user.spotipy_object_playlist_creation.playlist_add_items(
